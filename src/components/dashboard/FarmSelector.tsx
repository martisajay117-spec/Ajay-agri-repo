  ...
  return (
    <div className="relative inline-block text-left">
      <button
        ...
        className="flex items-center gap-2 px-3 py-1.5 h-8.5 bg-white hover:bg-slate-50 border border-slate-300 rounded-lg shadow-2xs text-xs font-semibold text-slate-800 transition-colors focus:outline-none focus:ring-2 focus:ring-slate-300"
      >
        <MapPin className="w-3.5 h-3.5 text-[#1e5128] shrink-0" />
      ...
      {isOpen && (
        <>
          ...
          />
          <div className="absolute left-0 mt-1 w-64 bg-white rounded-lg shadow-lg border border-slate-200 py-1.5 z-50 animate-in fade-in zoom-in-95 duration-100">
                ...
                return (
                  <button
                    ...
                    className={`w-full px-3 py-2 text-left text-xs flex items-center justify-between hover:bg-slate-50 transition-colors ${
                      isSelected ? 'bg-slate-100 text-slate-900 font-bold' : 'text-slate-700'
                    }`}
                  >
                    <div>
