  ...
  return (
    <>
      <div className="bg-white rounded-xl p-4 border border-slate-200 flex flex-col justify-between h-full min-h-[360px]">
        ...
        <div className="flex items-center justify-between py-2.5">
          <div>
            ...
            <div className="text-xs font-medium text-slate-700 mt-0.5 flex items-center gap-1.5">
              {renderWeatherIcon(current.iconType, 'w-3.5 h-3.5')}
          ...
          <div className="text-right">
            <div className="inline-flex items-center gap-1 px-2 py-1 rounded bg-[#1e5128]/10 text-[#1e5128] border border-[#1e5128]/20 text-[11px] font-mono font-semibold">
              <ShieldCheck className="w-3.5 h-3.5" />
              <span>ΔT {deltaT}°C • Spray Safe</span>
            </div>
            <div className="text-[10px] font-mono text-slate-400 mt-1">Wind &lt; 15 km/h limit</div>
          </div>
        </div>

        {/* Primary Sensor Row */}
        <div className="grid grid-cols-3 gap-2 text-center py-2 bg-slate-50 rounded-lg border border-slate-100 font-mono text-xs">
          <div className="p-1">
            <div className="text-[10px] text-slate-500 uppercase tracking-wider font-sans">Rel. Humidity</div>
          ...
          <div className="p-1 border-x border-slate-200">
            <div className="text-[10px] text-slate-500 uppercase tracking-wider font-sans">Barometer</div>
            <div className="font-bold text-slate-900 mt-0.5 tabular-nums">1012.4 hPa</div>
          </div>
          <div className="p-1">
            <div className="text-[10px] text-slate-500 uppercase tracking-wider font-sans">Precip. Risk</div>
        ...
        {/* 5-Day Forecast Row */}
        <div>
          <div className="text-[10px] uppercase font-bold tracking-wider text-slate-400 mb-1.5 flex justify-between">
            <span>5-Day Outlook</span>
            <span className="font-mono">Open-Meteo High-Res</span>
          </div>
          <div className="grid grid-cols-5 gap-1.5 text-center">
            {forecastDays.map((d, idx) => (
              <div key={idx} className="border border-slate-100 rounded-md p-1.5 bg-white">
                <span className="text-[10px] font-mono text-slate-500 uppercase block">{d.dayName}</span>
